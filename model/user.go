...
func applyUserIDStart(tx *gorm.DB, user *User) error {
	start := common.UserIDStart
	if start <= 1 || user.Id != 0 || !common.UsingMainDatabase(common.DatabaseTypeSQLite) {
		return nil
	}
	var maxId int
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			if err := applyUserIDStart(tx, user); err != nil {
				return err
			}
